...
ひらがなの文字内容と書体を条件として生成する、小型画像拡散モデルの実験リポジトリです。
...
PC上で基準モデルを成立させた後、蒸留・量子化・少ステップ化などを行い、最終的にはScratch上だけで推論することを目指します。
...
4. 成立した基準モデルをScratch向けに圧縮する

初期段階ではScratchのモデルサイズや計算量の制約を設けず、生成能力と条件設計の検証を優先します。
